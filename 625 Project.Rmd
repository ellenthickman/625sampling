...
Week6: Interclass Correlation Coefficient (ICC) for clustering
```{r}
# Calculate Roh from Previou Study
n_prev <- 7500
a_prev <- 150
...
```{r}
# Final summary table for the chosen sampling plan
summary_3x <- data.frame(
  variable = c("cig","mj","age"),
  roh = as.numeric(roh),
  m_opt_by_var = as.integer(m_opt),
  m_used = m_overall,
  a_used = a_overall,
  n_used = n_overall,
  deff_new = as.numeric(deff_new),
