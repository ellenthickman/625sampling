...
library(sampling)
selected_schools <- schoolframe_sorted %>%
  group_by(Region) %>%
  group_modify(~{
    
    data_h <- .x
    
    # total MOS in stratum
    MOS_h <- sum(data_h$tot_all)
    
    # number of schools to select in this stratum
    a_h_pair <- unique(data_h$a_h_pair)
    
    # inclusion probabilities
    pik <- a_h_pair * data_h$tot_all / MOS_h
    
    # select sample using systematic PPS
    s <- UPsystematic(pik)
...
selected_schools %<>%
  mutate(sufficient_size = (tot_all >= b_h))
...
selected_schools <- selected_schools %>%
  group_by(Region) %>%
  mutate(
    MOS_h = sum(tot_all)   # total MOS in stratum
  ) %>%
  ungroup()

selected_schools <- selected_schools %>%
  mutate(
    f1 = (a_h_pair * tot_all) / MOS_h
  )

selected_schools <- selected_schools %>%
  mutate(f2 = b_h / tot_all)
...
regcounts <- selected_schools %>%
  group_by(Region) %>%
  summarise(f_ji = sum(f2))
```
